    ...
    >
      ...
      >
        ...
        <span style={{ fontFamily: 'var(--font-mono)', color: '#cbd5e1' }}>
          {formatCurrency(row.p10)} – {formatCurrency(row.p90)}
        </span>
      </div>
      <div style={{ borderTop: '1px solid #1d2a44', paddingTop: '8px', marginTop: '8px' }}>
...
export function Step5_Summary({ onEditPlan }: Step5Props) {
  ...
  // Instant first paint, tween on updates only (matches LifetimeTimeline; errors.md row 10).
  ...
  const progressPct = Math.min(
    100,
    (res.medianPortfolio / Math.max(1, res.requiredPortfolio)) * 100
  );
        ...
        <FintechCard variant="info">
          <div className="mx-auto max-w-xl px-6 py-12 text-center space-y-4" role="status">
            <h2 className="text-2xl font-semibold text-text-primary">
              Your summary isn&apos;t ready yet
            </h2>
            <p className="text-base text-text-secondary">
              Add your current spending in <span className="font-semibold text-white">Step 1</span>{' '}
              and design your retirement lifestyle in{' '}
              <span className="font-semibold text-white">Step 2</span>. Once you&apos;ve set what
              you&apos;ll spend in retirement, your required portfolio, projection, success
              probability, and progress all appear here.
            </p>
            {onEditPlan && (
  ...
  return (
    <div className="space-y-8 print-container">
      ...
      {/* ── The details ── */}
      <h2 className="text-sm font-semibold text-text-secondary uppercase tracking-widest pt-2">
        The details
      </h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <MetricCard variant="info">
          ...
          <div className="text-xs text-text-secondary mt-2">
            {res.requiredPortfolio === 0 ? (
              <>
                Your guaranteed income (Social Security / pension) fully covers your{' '}
                {formatCurrency(retMonthlySpend)}/mo retirement spending — no investment portfolio
                required.
              </>
            ) : (
              <>
                In today&apos;s $ · ≈{formatLarge(res.requiredPortfolio)} in {retirementYear}. Funds{' '}
                {formatCurrency(retMonthlySpend)}/mo for {i.retDuration} yrs at{' '}
                {formatPercent(res.withdrawalRate * 100)} — lower spending in Step 2 to lower this.
              </>
            )}
          </div>
        </MetricCard>
        ...
        <MetricCard variant="success">
          ...
          <div className="text-xs text-text-secondary mt-2">
            Median (typical) outcome, in today's $ · ≈{formatLarge(res.medianPortfolio)} in{' '}
            {retirementYear}
          </div>
        </MetricCard>
      ...
      <FintechCard variant="info">
        ...
        <div className="flex justify-between text-xs text-text-secondary mb-2">
          <span>
            Projected (median, today's $):{' '}
            <span className="text-white font-semibold">{formatLarge(medianToday)}</span>
          </span>
          <span>
      ...
      <Reveal>
        <FintechCard variant="info">
          ...
          <p className="text-xs text-text-secondary mb-6">
            ...
            The luckiest outcomes run far higher and are left off the chart so the typical path
            stays readable
            {p75Today > 0 && (
              <>
                {' '}
          ...
          <div style={{ height: '420px' }}>
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                ...
                <Tooltip content={<NetWorthTooltip requiredPortfolio={res.requiredPortfolio} />} />
                ...
                <Area
                  ...
                  isAnimationActive={tween && !reduce}
            ...
            {[
              ...
              <div
                ...
                style={{
                  ...
                  borderRight: idx % 2 === 0 ? '1px solid #101828' : 'none',
      ...
      <Reveal>
        <FintechCard variant="warning">
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td
                        ...
                        style={{ color: offset > 0 ? '#10b981' : '#94a3b8' }}
                      >
                        {offset > 0 ? (
                          formatLarge(offset)
                        ) : (
                ...
                >
                  ...
                  <td className="py-3 px-4 text-right font-bold text-text-primary">
                    {formatCurrency(
                      (i.socialSecurity + i.pension + i.otherIncome) * res.inflationMult
                    )}
                  </td>
          ...
          <p className="mt-4 text-xs text-text-secondary leading-relaxed">
            How this is treated: Social Security keeps pace with inflation through its
            cost-of-living adjustment, so it holds its value. Pension and other income are shown at
            their {retirementYear} value but are not inflation-adjusted after that — most pensions
            have no cost-of-living raise, so their real spending power erodes through retirement.
            The success probability above already reflects that erosion; this offset is a
            simplified, retirement-year snapshot. All figures are pre-tax.
          </p>
        </FintechCard>
      </Reveal>
      ...
      >
        These projections are estimates. The success probability and ranges come from 1,000 Monte
        Carlo simulations that vary market returns each year and account for sequence-of-returns
        risk, so they are probabilities rather than promises. All figures are pre-tax and do not
        model taxes or fees. This tool is educational and not personalized financial advice;
        consider reviewing your plan annually with a Certified Financial Planner (CFP).
      </div>
    </div>
  );
}
